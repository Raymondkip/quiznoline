...
if(isset($_POST['name']) and isset($_POST['add_subcategory'])){
	...
	// common image file extensions
	if($_FILES['image']['error'] == 0 && $_FILES['image']['size'] > 0){
		if (!is_dir('subcategory')) {
			mkdir('subcategory', 0777, true);
		}
		$allowedExts = array("gif", "jpeg", "jpg", "png");
		// $extension = explode(".", $_FILES["image"]["name"]);
		// $extension = end($extension);
	...
	if(isset($_POST['sortable']) && $_POST['sortable'] == 'sortable'){
	    foreach($res as $category){
        	...
        	if(!empty($category["image"])){
                $options .="<li id='".$category["id"]."'><big>".$category["row_order"].".</big> &nbsp;<img src='subcategory/$category[image]' height=30 > ".$category["subcategory_name"]."</li>";
            }else{
...
if(isset($_POST['import_questions']) && $_POST['import_questions']==1){
	...
	if($_FILES["questions_file"]["size"] > 0)
    {
        ...
        while (($emapData = fgetcsv($file, 10000, ",")) !== FALSE)
        {
			...
			$count++;
			if($count>1){
				if(!empty($emapData[0]) && !empty($emapData[2])){
...
if(isset($_POST['get_categories_of_language']) && $_POST['get_categories_of_language'] != '' ) {
	...
	if(isset($_POST['sortable']) && $_POST['sortable'] == 'sortable'){
	    foreach($res as $category){
        	...
        	if(!empty($category["image"])){
                $options .="<li id='".$category["id"]."'><big>".$category["row_order"].".</big> &nbsp;<img src='category/$category[image]' height=30 > ".$category["category_name"]."</li>";
            }else{
