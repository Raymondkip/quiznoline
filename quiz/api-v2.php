...
if(isset($_POST['access_key']) && isset($_POST['get_categories'])){
	...
	*/
	...
	if(isset($_POST['id'])){
		...
		$sql = "SELECT *,(SELECT @no_of_subcategories := count(`id`) from subcategory s WHERE s.maincat_id = c.id and s.status = 1 ) as no_of, if(@no_of_subcategories = 0, (SELECT @maxlevel := MAX(`level`+0) from question q WHERE c.id = q.category ),@maxlevel := 0) as `maxlevel` FROM `category` c WHERE c.id = $id ORDER By c.row_order + 0 ASC";
		$db->sql($sql);
		$result = $db->getResult();
		// print_r($result);
		if (!empty($result)) {
		...
		$sql = "SELECT *,(SELECT @no_of_subcategories := count(`id`) from subcategory s WHERE s.maincat_id = c.id and s.status = 1 ) as no_of, if(@no_of_subcategories = 0, (SELECT @maxlevel := MAX(`level`+0) from question q WHERE c.id = q.category ),@maxlevel := 0) as `maxlevel` FROM `category` c ORDER By c.row_order ASC";
		$db->sql($sql);
		$result = $db->getResult();
		// print_r($result);
		if (!empty($result)) {
...
if(isset($_POST['access_key']) && isset($_POST['get_subcategory_by_maincategory'])){
	...
	*/
	...
	if(isset($_POST['main_id'])){
		...
		$sql = "SELECT *,(select max(`level` + 0) from question where question.subcategory=subcategory.id ) as maxlevel FROM `subcategory` WHERE `maincat_id`='$id' and `status`=1 ORDER BY row_order + 0 ASC";
		$db->sql($sql);
		$result = $db->getResult();
		// print_r($result);
		if (!empty($result)) {
	...
	*/
	...
	if(isset($_POST['language_id']) && !empty($_POST['language_id'])){
		...
		$sql = "SELECT *,(SELECT @no_of_subcategories := count(*) from subcategory s WHERE s.maincat_id = c.id and s.status = 1 ) as no_of, 
			...
			FROM `category` c where `language_id` = ".$language_id." ORDER By c.row_order ASC";
		$db->sql($sql);
		$result = $db->getResult();
		// print_r($result);
		if (!empty($result)) {
